function [p,I] = salt2(spt_baseline,spt_test,dt,wn)
...
for t = 1:nmbn:st   % loop through baseline windows
    for k = 1:tno   % loop through trials
        ...
        if ~isempty(pki)
            ...
            lsi(k,next) = NaN;   % 0 if no spike in the window
        end
    end
    slsi(:,next) = sort(lsi(:,next));   % sorted latencies
    hst = hist(slsi(:,next),edges);
    hlsi(:,next) = hst(1:end-1);   % latency histogram  (many '0' at bin 1)
