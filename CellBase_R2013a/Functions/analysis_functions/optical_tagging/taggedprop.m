...
for k = 1:NumCells
    ...
    try
        SALT_p= Hindex(findcellpos(cellid)); % already calculated by TAGGING, so pass it on
        SALT_diff=D_KL(findcellpos(cellid));
        main(cellid,issave,resdir,SALT_p,SALT_diff)  % every analysis
    catch ME
...
% -------------------------------------------------------------------------
function main(cellid,issave,resdir,SALT_p,SALT_diff)
...
[~, waveprop]= plotwaveforms(cellid,'spont',true,'maxnum',5000); % 'maxnum',  2000 - maximum number of spikes to plot

save([resdir '\TAGGEDPROP_' regexprep(cellid,'\.','_') 'spkwidth.mat'],...
        'waveprop');
...
[lID_amp lLr_amp] = lightcluster(cellid,'feature_names',{'feature_Peak','feature_Energy'},...
    'stim_period',[A1 A2]);
[lID_PC lLr_PC] =lightcluster(cellid,'feature_names',{'feature_Peak','feature_WavePC1'},...
    'stim_period',[A1 A2]);
% % HD = figure;
% % axes;
...
if issave
    save([resdir '\TAGGEDPROP_' regexprep(cellid,'\.','_') '.mat'],...
        ...
        'E_pulseon','L_pulseon','J_pulseon','B_pulseon','M_pulseon','lim_act1','lim_act2',...
         'Wp_act', 'MinRate', 'lim_inh1', 'lim_inh2', 'Wp_inh',... 
        'E_burston','L_burston','J_burston','B_burston','M_burston',...
        'lLr_amp','lID_amp','lLr_PC','lID_PC');
