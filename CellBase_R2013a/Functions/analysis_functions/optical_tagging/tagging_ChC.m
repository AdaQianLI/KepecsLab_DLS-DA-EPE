...
NumCells = length(I);
feature_names1 = {'feature_Peak','feature_Energy'};
feature_names2 = {'feature_WavePC1','feature_Energy'};
for k = 1:NumCells
    cellid = I{k};
    ...
    try
        ...
        if isstim
            ...
            % Tagging index
            [Hindex_act D_KL_act Hindex_inh D_KL_inh] = nbisstim(cellid);
        ...
        if issave
            ...
            % Save
            save([resdir fs 'TAGGING_' regexprep(cellid,'\.','_') '.mat'],...
                'Lr_amp','ID_amp','Lr_PC','ID_PC','valid_channels',...
                'Hindex_act','D_KL_act','Hindex_inh','D_KL_inh','R')
%              xlsname = [resdir fs regexprep(cellid,'\.','_') '.xls'];         
            % Write to Excel
...
% %             Hindex_act_xls = formatforExcel(Hindex_act);
% %             D_KL_act_xls = formatforExcel(D_KL_act);
% %             Hindex_inh_xls = formatforExcel(Hindex_inh);
% %             D_KL_inh_xls = formatforExcel(D_KL_inh);
...
% %             xlswrite(xlsname,Hindex_act_xls,'sheet1',['F' num2str(k)])
% %             xlswrite(xlsname,D_KL_act_xls,'sheet1',['G' num2str(k)])
% %             xlswrite(xlsname,Hindex_inh_xls,'sheet1',['H' num2str(k)])
% %             xlswrite(xlsname,D_KL_inh_xls,'sheet1',['I' num2str(k)])
% %             xlswrite(xlsname,R_xls,'sheet1',['J' num2str(k)])
        end
