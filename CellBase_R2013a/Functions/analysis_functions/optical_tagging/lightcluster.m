function [DM LrC Pref Pref2 valid_channels] = lightcluster(cellid,selts_evoked,varargin)
...
addParameter(prs,'feature_names',{'Energy'},@(s)iscell(s)|ischar(s))  % names of MClust features to use
...
% selts_evoked = extractSegSpikes(spk,tsegs_evoked);   % find putative stimualated spikes
selts_evoked = unique(selts_evoked);  % in very few cases, a spike time is registered twice
[junk,junk2,evoked_inx] = intersect(selts_evoked,all_spikes);  % get indices for light-evoked spikes
...
% -------------------------------------------------------------------------
function calculate_features(sessionpath,propfn_path,basename,valid_channels)
