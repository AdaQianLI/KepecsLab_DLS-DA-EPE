function [p,I] = salt2_inh(spt_baseline,spt_test,dt,wn)
...
for t = 1:nmbn:st   % loop through baseline windows
    for k = 1:tno   % loop through trials
        ...
        pki = find(cspt,1,'last');   % first spike in the window
        if ~isempty(pki)
...
%     cspt_comp=cspt(:,21:end);    % compare last spike after light stimulation
    pki = find(cspt,1,'last');   % first spike in window
    if ~isempty(pki)
