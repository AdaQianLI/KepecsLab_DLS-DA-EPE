...
%       'issave', true/false - controls saving behavior; plots and
...
addParamValue(prs,'issave',true,@islogical)   % control saving behavior
...
% Directories
global DATAPATH
resdir = [DATAPATH 'acg/'];  % results directory
...
for iC = 1:numCells   % loop through the cells
    cell = cellids{iC};
    try
        tseg = findSegs3(cell,'segfilter','stim',...
            'margins',[0 0]);  % find time segments
