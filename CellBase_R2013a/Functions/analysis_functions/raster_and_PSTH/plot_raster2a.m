...
default_args.EventMarkerWidth = 0.02;  % default 0.008
default_args.EventMarkerHeight = 20;  % default 2
default_args.SpikeWidth = 4; %0.5,2
            ...
            if ~iscell(ev_windows)
                ...
                if ~isempty(ok_spikes)
                    if size(ok_spikes,1) == 1
                        line([ok_spikes; ok_spikes],[(nt(iTRIAL)-thinkness)*ones(1,length(ok_spikes)); (nt(iTRIAL)+thinkness)*ones(1,length(ok_spikes))],'Color','k','LineWidth',par.SpikeWidth);
                    else
                        line([ok_spikes ok_spikes]',[(nt(iTRIAL)-thinkness)*ones(1,length(ok_spikes)); (nt(iTRIAL)+thinkness)*ones(1,length(ok_spikes))],'Color','k','LineWidth',par.SpikeWidth);
                    end
                end
                ...
                for k = 1:NUMlicks
                    ...
                    if ~isempty(ok_spikes{k})
                        if size(ok_spikes{k},1) == 1
                            ...
                            lny = [lny [(nt(cntr)-thinkness)*ones(1,length(ok_spikes{k})); (nt(cntr)+thinkness)*ones(1,length(ok_spikes{k}))]];
                        else
                            lnx = [lnx [ok_spikes{k} ok_spikes{k}]'];
                            lny = [lny [(nt(cntr)-thinkness)*ones(1,length(ok_spikes{k})); (nt(cntr)+thinkness)*ones(1,length(ok_spikes{k}))]];
                        end
            ...
            if ~iscell(ev_windows)
                if ~iscell(EventTimes)
                    ...
                    if length(find(et==0)) == NumTrials(iCOMP)
                        line([0 0],[1 NumTrials(iCOMP)],'Color',par.ShowEventsColors{iE},'LineWidth',1); %1
                    else
...
if par.RasterIDbarWidth ~= 0
    ...
    for iP = 1:NumParts
        ...
        if isfield(par,'Colors2')
            set(pb(iP),'EdgeColor',par.Colors2{jP});
        else
