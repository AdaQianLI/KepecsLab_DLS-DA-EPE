function [r,Eff,Lat,Jitter]=Stat_spikeshape(cellid,ADBitVolts,varargin)
...
addParameter(prs,'window',[-0.5 1],@(s)isnumeric(s)&isequal(length(s),2)) 
addParameter(prs,'win',[-0.25 0.25],@(s)isnumeric(s)&isequal(length(s),2)) 
addParameter(prs,'spon_window',[-0.1 0.1],@(s)isnumeric(s)&isequal(length(s),2)) 
addParameter(prs,'baselinewin',[-0.25 0],@(s)isnumeric(s)&isequal(length(s),2)) 
addParameter(prs,'testwin',[0 0.1],@(s)isnumeric(s)&isequal(length(s),2)) 
...
addParameter(prs,'idx',1,@isnumeric)
addParameter(prs,'relative_threshold',0.2,@isnumeric)
addParameter(prs,'isadaptive',false,@(s)islogical(s)|ismember(s,[0 1]))   % control displaying rasters and PSTHs
addParameter(prs,'display',false,@(s)islogical(s)|ismember(s,[0 1]))
addParameter(prs,'TriggerName','PulseOn',@ischar)   % reference event
addParameter(prs,'SortEvent','PulseOn',@ischar)   % reference event
addParameter(prs,'LastEvents','',@ischar)   % reference event
addParameter(prs,'event','PulseOn_chr2',@ischar)   % reference event
addParameter(prs,'Partitions','all',@ischar)   % reference event 'Partitions',   
addParameter(prs,'eventtype','stim',@(s)ischar(s)&&(strncmp(s,'stim',4)||strncmp(s,'trial',4)))   % event type - stim or trial
...
%     'BurstPSTH'             'off';...  
%     'raster_width',          0.5;...     % chr2 0.5  jaws 2
%     'idx',                   1;...
...
% Spike times
alltrials = 1:size(SP.event_stimes{g.idx},2);
...
%%
% spsth = binraster2selectivitytime(binraster,COMPTRIALS,valid_trials);



% Calculate PSTH
if ~g.isadaptive
    [~, spsth, spsth_se] = binraster2psth(binraster,g.dt,g.sigma,COMPTRIALS,valid_trials);
else
    [~, spsth, spsth_se] = binraster2apsth(binraster,g.dt,g.sigma,COMPTRIALS,valid_trials);   % adaptive PSTH
end
...
% % Plot the raster
% if g.rasterPlot == 1
% fhandle0 = plot_raster2a(stimes,time,valid_trials,COMPTRIALS,mylabels,EventTimes,window_margin,ev_windows,sort_var,g,'SpikeWidth',raster_width,'Colors',{mycolors},'Colors2',{mycolors2},'NumTrials2Plot',g.Num2Plot);
% % set(fhandle0,'LineWidth',5);%%%%%
% if isfield(g,'Legend'),
%     mylabels = g.Legend;
% end
% end
...
%         end
%     end
%     fhandle=plot_timecourse(time,spsth,spsth_se,g,'FigureNum',fhandle0(end),'Colors',{mycolors},'LineStyle',{mylinestyle},'Legend',{mylabels},'XLabel',XLabel,'YLabel',YLabel);
%     axis tight
% end
...
%%
         psthstat = psth_stats(cellid,binraster,spsth,'sigma',g.sigma,'dt',g.dt,'win',g.win,'plotwin',g.plotwin,...
        'baselinewin',[g.win(1) 0],'testwin',[0.001 g.win(2)],'display',0,'relative_threshold',0.2); 
    
        r= plotwaveforms_2(cellid,psthstat,ADBitVolts,'event',g.TriggerName,'spon_window',[g.win(1) 0],'maxnum',5000);
        
        
        [Eff,Lat,Jitter]=reliability_latency_jitter(cellid,psthstat,'event',g.TriggerName,'event_type','stim','window',g.win,'dt',g.dt,'sigma',g.sigma,'baselinewin',[g.win(1) 0],'testwin',[0.001 g.win(2)]);
