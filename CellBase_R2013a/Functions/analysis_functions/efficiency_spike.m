function E= efficiency_spike(cellid,varargin)
...
addRequired(prs,'stimes',@(s)isnumeric(s)|iscell(s))   % time stamps
...
for k = 1:tno
    if isequal(event_type,'tria') && ~isequal(event,'TrialStart')
        pr = g.stimes(k) - (VE.(g.event)(valid_trials) + VE.TrialStart(valid_trials));     % trial events other than TrialStart are stored relative to TrialStart
    else
        pr = g.stimes(k) - VE.(g.event)(valid_trials);
    end
    evinx(k) = find(pr>0,1,'last');
end
% spnodist = arrayfun(@(s)sum(evinx==s),1:length(valid_trials));   % number of spikes for all valid trials
evinx= unique(evinx,'first');    % first spikes after light pulse
% stimes1st = g.stimes(evinx);   % spike times including first spikes only in each trial

% Efficiency
allev = VE.(g.event)(valid_trials);
