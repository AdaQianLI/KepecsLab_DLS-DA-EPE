function [H,F,J]= plot_mclust_projections2_VIP(cellid,selts_evoked,varargin)
...
addParameter(prs,'feature_names',{'feature_Peak'},@(s)iscell(s)|ischar(s))  % names of MClust features to use
addParameter(prs,'event','PulseOn_chr2',@ischar)   % reference event
addParameter(prs,'marker','.')  % marker for the plots
addParameter(prs,'marker_size',2,@isnumeric)  % marker size for the plots
addParameter(prs,'usefastplot',true,@(s)islogical(s)|ismember(s,[0 1]))  % fast plotting (no zoom)
...
Datfile = cellid2fnames(cellid,'dat');
all_spikes = mClustTrodes4LoadingEnginey(Datfile);
...
hold on
    ...
    if g.usefastplot
     H{k}= fastplot(xdata,ydata,[0.8 0.8 0.8],g.marker,g.marker_size);   % grey, all spikes from multi units, same rec. site 
    else
     H{k}= slowplot(xdata,ydata,[0.8 0.8 0.8],g.marker,g.marker_size);
    end
...
%     end
    ...
    if g.usefastplot
       F{k}= fastplot(xdatai,ydatai,[255 204 0]/255,g.marker,g.marker_size*2);  %orange, all spikes from single unit being selected
    else
       F{k}=slowplot(xdatai,ydatai,[255 204 0]/255,g.marker,g.marker_size*2);
    end
    ...
    if g.plotlightspikes
        ...
        if g.usefastplot
        J{k}=fastplot(xdatai,ydatai,[0 153 255]/255,'.',g.marker_size*3);  %blue, light evoked spikes
        J{k}=slowplot(xdatai,ydatai,[0 153 255]/255,'.',g.marker_size*3);
        uistack(J{k},'top')
        end
    end
