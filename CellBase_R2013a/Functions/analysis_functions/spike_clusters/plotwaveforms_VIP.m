function [H,Y,stimwin,R,mean_spont,mean_evoked,selts_evoked] = plotwaveforms_VIP(cellid,stats,ADBitVolts,varargin)
...
selts_evoked = extractSegSpikes_VIP(cellid,tsegs_evoked);   % find evoked spikes
...
selts_evoked=unique(selts_evoked);
    ...
    wave_evoked = extractSpikeWaveform_VIP(cellid,selts_evoked,'chans','all');  % get waveforms for the extracted spikes
...
if g.spont || g.correlation
    ...
    selts_spont = extractSegSpikes_VIP(cellid,tsegs_spont);     % extract spontaneous spikes
    selts_spont=unique(selts_spont);
    
    wave_spont = extractSpikeWaveform_VIP(cellid,selts_spont,'chans','all');    % get waveforms for the extracted spikes
