...
default_args = {...
    ...
    'min_int',      0;...            % minimum interval of time segment (secs)
    'max_int',      1;...          % max interval
...
function [prestim_segs g] = beh_only(g,cellid)

% behavior events
try
    % Load stimulus events
    SE = loadcb(cellid,'TrialEvents');

    % Pre-stimulus segments: from the offset of a pulse to the onset of the next
        prestim_segs = [SE.Type1Reward+0.5; SE.Type1Reward+2];
catch
    disp([cellid ': There was no Behavior protocol for this session.'])
    prestim_segs = [];
end
...
try
    ...
    if ~isempty(SE.PulseOn)
        prestim_segs = [SE.PulseOff; SE.PulseOn];
    else
        prestim_segs = [];
    end
catch
...
try
    ...
    SE = loadcb(cellid,'StimEvents');
        ...
        a = unique(SE.PulseOn(~isnan(SE.PulseOn)));
        stim_begin=a(1:length(a)-1);
        stim_end = a(2:length(a));% 2 seconds after the last pulse
catch
    disp([cellid ': There was no stim protocol for this session.'])
    stim_begin = [];
    stim_end = [];
...
try
    ...
    a = unique(TE.TrialStartTimestamp(~isnan(TE.TrialStartTimestamp)));
    behav_begin=a(1:length(a)-1);
    behav_end = a(2:length(a));
...
try
    ...
    stim_segs = [SE.BurstOn+1; SE.BurstOn+1.5];
catch
    disp([cellid ': There was no stim protocol for this session.'])
    stim_segs = [];
end
...
try
    ...
    all_behav_stims = unique(TE.TrialStartTimestamp(~isnan(TE.TrialStartTimestamp)));
    ...
    all_behav_segs = reshape(sort([TE.TrialStart(1) all_behav_stimsegs(:)' TE.TrialEnd(1)]),2,[]);  % behav. segments excluding stimulus segments
catch
    disp([cellid ': There was no behavioral protocol for this session.'])
    all_behav_segs = [];
    all_behav_stimsegs = [];
...
try
    ...
    % Druing stimulation protocols: from pulse onset, using light activation duration argument
    TE.LightStimulation2 = TE.Stimulus_Type1Reward(~isnan((TE.Stimulus_Type1Reward)));
...
if isequal(sesstype,{'Type1Reward'})
    alignevent_fa = 'Stimulus_Type1Reward';
elseif  isequal(sesstype,{'Type1TimeOut'})
    alignevent_fa = 'Stimulus_Type1TimeOut';
elseif isequal(sesstype,{'Type2AvoidSucc'})
    alignevent_hit = 'Stimulus_Type2AvoidSucc';
else
    alignevent_hit = 'Stimulus_Type2Punish';
end
