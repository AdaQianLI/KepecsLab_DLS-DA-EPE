...
addParamValue(prs,'cellids',{},@iscell) 
...
%             disp('addanalysis:analysisExists','ADDANALYSIS: Analysis already performed in column %d. Delete first.')
%         end
%     end
% end
...
%     if prevanal
%         disp('addanalysis:propertyExists','ADDANALYSIS: Property name already exists at position %d. Delete first or choose a different one.')
%     end
% end
...
% ANALYSES(NewAnal).cellid  = g.cellids;
...
% Execute analysis
 ...
 for i=1:size(cellid,2)
    cellnum = find(strcmp(CELLIDLIST,cellid(i))); % loop through all cells
    disp(cellid(i))
    print_progress(cellnum,round(NumCells/100),5);   % progress indicator
     arglist =[cellid(i) pargl(:)'];  % input arguments for the analysis
%          arglist = [char(CELLIDLIST{cellnum}) g.mandatory pargl(:)'];  
    try
