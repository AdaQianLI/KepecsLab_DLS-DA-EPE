function Data=calcwaveformfeatures(waveforms,varargin)
...
[II,YY]=find(meanWV==max(temp));% this may not be the largest channel since
...
if ~isempty(wei)
    HyperEndI = XX(wei);
else HyperEndI = XX(end);
end % in case it does not return to baseline
...
Width =(ValleyI-PreValleyI)*dt*10000; % (per/sec)*100*10000 i.e. microsec
Hyper = (HyperEndI-HyperBeginI)*dt*10000;
Width2 = (HyperEndI-PeakI)*dt*10000;
% Width2 = (HyperEndI-PreValleyI)*dt*10000;
