...
% NIDAQ_PHOTOMETRY_JOYSTICK_V3
% Helper that initializes the NI-DAQ session, starts each trial's
% acquisition in the background, stops the session, and packages raw
% photometry and joystick trajectory signals for saving.
global nidaq S
...
nidaq.ai_channels              = {'ai0','ai1','ai2','ai3'}; % ai0/ai1 = photometry inputs, ai2/ai3 = joystick X/Y inputs.
nidaq.ai_data                  =[];

% Define parameters for analog outputs (important for LED modulation
nidaq.ao_channels                 = {'ao0','ao1'}; % LED modulation outputs.
nidaq.ao_data                     = [];
...
% lh{2} = nidaq.session.addlistener('DataRequired', @(src,event) src.queueOutputData(nidaq.ao_data));
    ...
    case 'WaitToStart'
%% Prepare one trial of NI-DAQ recording
    nidaq.ai_data = [];
...
nidaq.session.queueOutputData(nidaq.ao_data);  % Queue the full trial's LED reference waveforms.
...
nidaq.session.startBackground();    % Acquire asynchronously while the Bpod state machine runs.
...
%% STOP NIDAQ
    ...
    nidaq.session.outputSingleScan(zeros(1,length(nidaq.ao_channels))); % drop output back to 0
     case 'Save'
%% Package raw signals for saving and downstream online processing
photometry470Data=[];
photometry565Data=[];
