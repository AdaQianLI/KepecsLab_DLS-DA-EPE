function figData=Online_NidaqPlot_8class_565(action,Phase,trialsResNames,figData,newData565,nidaqRaw,curTrialType)
% ONLINE_NIDAQPLOT_8CLASS_565
% Online figure for the second photometry channel (565 nm or alternate
% demodulated channel): current raw trace plus class-wise averages.
global BpodSystem S
...
try
    close 'Online Nidaq Plot_565nm';
end
%% Create Figure
figPlot=figure('Name','Online Nidaq Plot_565nm','Position', [650 120 600 700], 'numbertitle','off'); %[left bottom width height]
hold on
...
allData(:,dataSize+1)=newData565(1:factor:end,3);
...
set(figData.meanplot(curTrialType), 'XData',newData565(1:factor:end,1),'YData',meanData,'LineWidth',MeanThickness);
set(curSubplot,'NextPlot','add');
plot(newData565(1:factor:end,1),newData565(1:factor:end,3),'-k','parent',curSubplot);
