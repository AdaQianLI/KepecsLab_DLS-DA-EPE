...
% ONLINE_POSITION_8CLASS
% Online figure for joystick trajectories. It shows the latest trajectory
% and condition-specific overlays/means across trials.
global BpodSystem S
...
%% Create Figure
figPlot=figure('Name','Online Position Plot','Position', [10 50 250 1000], 'numbertitle','off');  %[left bottom width height]
hold on
...
for i=1:size(trialsNames_8class,2)
    ...
    xlabel(labelx); 
    ylabel(labely);
    set(motorsubplot(i),'XLim',traj_range,'YLim',traj_range);
...
set(curSubplot0,'XLim',traj_range,'YLim',traj_range);%'YLim',[miny maxy]
