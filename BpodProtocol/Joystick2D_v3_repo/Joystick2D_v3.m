...
% photometry acquisition, and online plotting.
%
% Trial structure used in the baseline null-field task:
%   4 s pre-cue period -> 0.5 s cue -> 1 s delay -> 5.5 s response window
% Successful trials then enter a 1 s pre-reward delay followed by valve
% opening. Premature reaches before the go period trigger the warning
...
%   3 = success cue
%   4 = early-reach warning buzzer (three brief beeps)
...
%% Assemble the Bpod state matrix for the current trial
    ...
    sma=AddState(sma,'Name', 'TooEarly',...
        ...
        'OutputActions', {'SoftCode',4}); % Premature reach: play warning buzzer and skip to the between-trial interval.
