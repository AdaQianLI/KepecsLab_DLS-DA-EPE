...
% ONLINE_NIDAQDEMOD_JOYSTICK
% Demodulate the raw photometry channel using the LED reference waveform,
% downsample it for plotting, and compute dF/F relative to the configured
% baseline window.
global BpodSystem S
...
%% Low-pass filter setup for demodulated signals
    lowCutoff = lowCutoff/(sampleRate/2); % normalized CutOff by half SampRate (see doc)
    [b, a,k] = butter(10, lowCutoff, 'low'); 
    [sos, g] = zp2sos(b,a,k);
if mod
    %% Prepare the LED reference signal and a 90-degree phase-shifted copy
...
%% Quadrature demodulation and low-pass filtering
...
%% Scale by the modulation amplitude
...
%% Downsample to the plotting rate and pad to the expected trial length
...
%% Package the raw channel on the original sample grid
